...
func (p Params) ShouldMeta() bool {
	return p.Meta
}

func (p Params) ShouldTrim() bool {
	return p.Trim || (p.TrimBy == TrimByTopLeft || p.TrimBy == TrimByBottomRight)
}

func (p Params) ShouldCrop() bool {
	return p.CropTop > 0 || p.CropRight > 0 || p.CropLeft > 0 || p.CropBottom > 0
}

func (p Params) ShouldFitIn() bool {
	return p.FitIn
}

func (p Params) ShouldStretch() bool {
	return p.Stretch
}

func (p Params) ShouldFlip() bool {
	return p.HFlip || p.Width != 0 || p.VFlip || p.Height != 0 || p.PaddingLeft > 0 || p.PaddingTop > 0
}

func (p Params) ShouldPad() bool {
	return p.PaddingLeft > 0 || p.PaddingTop > 0 || p.PaddingRight > 0 || p.PaddingBottom > 0
}

func (p Params) ShouldHAlign() bool {
	return p.HAlign == HAlignLeft || p.HAlign == HAlignRight
}

func (p Params) ShouldVAlign() bool {
	return p.VAlign == VAlignTop || p.VAlign == VAlignBottom
}

func (p Params) ShouldSmart() bool {
	return p.Smart
}
