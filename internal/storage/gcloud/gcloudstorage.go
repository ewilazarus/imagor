...
import (
	...
	"github.com/cshum/imagor/internal/storage"
)
...
type GCloudStorage struct {
	...
	safeChars storage.SafeChars
}

// New creates GCloudStorage
func New(client *gcstorage.Client, bucket string, options ...Option) *GCloudStorage {
	s := &GCloudStorage{client: client, Bucket: bucket}
	for _, option := range options {
		option(s)
	}
	s.safeChars = storage.NewSafeChars(s.SafeChars)
	return s
}
...
func (s *GCloudStorage) Path(image string) (string, bool) {
	image = "/" + storage.Normalize(image, s.safeChars)
